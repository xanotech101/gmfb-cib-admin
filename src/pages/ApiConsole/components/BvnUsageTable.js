...
export const BvnUsageTable = ({ data, initialSerialNumber }) => {
  ...
  const {
    ...
    queryFn: () => apiUsageService.getApi(user?._id, { date: year, requesttype: 'Bvn' })
  });
  const sumOfRequests = useMemo(() => {
    return thirdparty?.analytics?.length === 0 || thirdparty?.analytics === undefined
      ? '0 Count'
      : thirdparty?.analytics?.reduce((total, obj) => `${total + obj?.numberOfRequests} Count`, 0);
  }, [thirdparty]);
            ...
            {isLoading || isFetching ? (
              ...
              <div className="mt-7">
                <dl className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-3">
                  ...
                  <div className="sm:col-span-3 flex justify-between border-b pb-4">
                    <dt className="text-sm font-medium text-gray-500">Total Bvn Count</dt>
